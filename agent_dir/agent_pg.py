...
class AgentPG(Agent):
    ...
    def update(self):
        ...
        # compute loss
        loss = []
        for idx, (log_prob, reward) in enumerate(zip(self.log_probs, self.rewards)):
            loss.append(-log_prob * reward * math.pow(self.gamma, idx))
    ...
    def train(self):
        # logging
        import logging
        logging.basicConfig(filename="pg_reward.log", level=logging.INFO)
        ...
        for epoch in range(self.num_episodes):
            ...
            if epoch % self.display_freq == 0:
                logging.info("{},{}".format(epoch, avg_reward))

...
class ActorCritic(nn.Module):
    ...
    def _forward_rnn(self, x, hiddens, masks):
        ...
        x = x.view(-1, hiddens.size(0), hiddens.size(1))
        masks = masks.view(-1, hiddens.size(0),1)
        outs = []
        for xt, mask in zip(x, masks):
            xt = xt.unsqueeze(0)
            hiddens = hiddens*mask
            out, hiddens = self.rnn(xt, hiddens.unsqueeze(0))
            hiddens = hiddens.squeeze(0)
            outs.append(out)

        x = torch.stack(outs, 0).view(-1, hiddens.size(1))
